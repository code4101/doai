...
from pyxlpr.ai.torch import TrainingSampler, XlPredictor, ClasEvaluater  # 封装了一些通用组件，简化开发
...
STATE_FILE = 'mnist/resnet18_model.pth'  # 计划存储权重文件的路径
...
class ResNet18(nn.Module):
    def __init__(self, pretrained=False, progress=True, num_classes=1000, **kwargs):
        super().__init__()
        self._resnet = resnet18(pretrained=pretrained, progress=progress, num_classes=num_classes,
                                **kwargs)
...
def eval():
    predictor = XlPredictor(ResNet18(num_classes=NUM_CLASSES), STATE_FILE, DEVICE, batch_size=IMS_PER_BATCH)
